package com.adhi.cowboyshoot.manager
...
class ComputerEnemyCowboyGameManager(
    ...
    override fun initGame() {
        ...
        playerTwo = Player(PlayerSide.PLAYER_TW0, PlayerState.IDLE, PlayerPosition.MIDDLE)
        notifyPlayerDataChanged()
        setGameState(GameState.STARTED)
    }

    private fun notifyPlayerDataChanged() {
        listener.onPlayerStatusChanged(playerOne, getPlayerOneDrawableByState(playerOne.playerState))
        listener.onPlayerStatusChanged(playerTwo, getPlayerTwoDrawableByState(playerTwo.playerState))
    }
